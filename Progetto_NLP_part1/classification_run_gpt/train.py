...
def set_up_arguments(epochs, model, loss, device):
    ...
    scheduler = CosineAnnealingWarmRestarts(torch.optim.AdamW(model.parameters(),lr=5e-5,eps=1e-8), 10000) 
...
def main(rank: int):
    ...
    args, tokenizer = set_up_arguments(NUM_EPOCHS, GPT2ForSequenceClassification.from_pretrained("gpt2", num_labels=5), nn.CrossEntropyLoss(), int(os.environ["LOCAL_RANK"]))#(NUM_EPOCHS, BertForRegressionSigmoid(), nn.MSELoss(), int(os.environ["LOCAL_RANK"]))
    if TESTING:
