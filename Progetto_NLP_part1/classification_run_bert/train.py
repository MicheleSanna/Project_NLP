...
def set_up_arguments(epochs, model, loss, device):
    ...
    dataset = JsonFastClassificationDataset(TRAIN_DATASET, "scattered_dataset/ScatteredTenMillions", N_FILE, tokenizer)#9999018
    ...
    scheduler = StepLR(torch.optim.AdamW(model.parameters(),lr=5e-5,eps=1e-8), 1, gamma=0.2) 
...
def main(rank: int):
    ...
    args, tokenizer = set_up_arguments(NUM_EPOCHS, BertForSequenceClassification.from_pretrained("bert-base-cased", num_labels=5), nn.CrossEntropyLoss(), int(os.environ["LOCAL_RANK"]))#(NUM_EPOCHS, BertForRegressionSigmoid(), nn.MSELoss(), int(os.environ["LOCAL_RANK"]))
    if TESTING:
    ...
    #model = trainer.train()
    if rank==0:
        trainer.model = torch.load("Final_model.pth")
        torch.save(trainer.model.module.state_dict(), "final_state.sav")
