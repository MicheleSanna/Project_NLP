    ...
    def __iter__(self):
            ...
            with open(self.file) as f:
                for sample_line in f:
                    ...
                    yield (sample['overall']), self.tokenizer(sample['reviewText'], add_special_tokens=True, max_length=500, truncation='longest_first',return_tensors='pt')
        ...
        with open(self.filename + str(int(idx//(MAXDIM/self.nFile))) + ".json", 'r') as f:
            for line in f:
                if place != idx%(MAXDIM/self.nFile):
                    ...
                    sample = json.loads(line)
                    return int((sample['overall'] - 1)/4), self.tokenizer(sample['reviewText'], add_special_tokens=True, padding='max_length', max_length=500, truncation='longest_first',return_tensors='pt')
