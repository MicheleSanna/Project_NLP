...
def conditions(sample):
    if sample['overall'] == 1.0 or sample['overall'] == 5.0:
    ...
    with open(file) as f: 
        for line in f:
            if n1star + n5star <= dim:
                ...
                printFlag = False
                try:
                    flag = conditions(sample)
                except:
                    eccezioni = eccezioni +1
                    print("Eccezione: ", eccezioni)
                    continue
                if flag:
...
#print("START")
UniqueFile(20000, "TenMillions.json", "LowPerplexityDataset_bert.json")
